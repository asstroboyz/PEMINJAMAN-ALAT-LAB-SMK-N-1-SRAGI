    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3 d-flex align-items-center justify-content-between flex-wrap">
            ...
            <div class="d-flex align-items-center">
                <a href="<?php echo base_url('admin/tambahPeminjaman')?>" class="btn btn-sm btn-primary mr-2">
                    <i class="fa fa-plus"></i> Tambah Peminjaman
                </a>
                <div class="btn-group">
                    <a href="<?php echo base_url('admin/peminjaman?status=all')?>" class="btn btn-sm <?php echo $status == 'all' ? 'btn-info' : 'btn-outline-secondary'?>">Semua</a>
                    <a href="<?php echo base_url('admin/peminjaman?status=diproses')?>" class="btn btn-sm <?php echo $status == 'diproses' ? 'btn-info' : 'btn-outline-secondary'?>">Diproses</a>
                    <a href="<?php echo base_url('admin/peminjaman?status=selesai')?>" class="btn btn-sm <?php echo $status == 'selesai' ? 'btn-info' : 'btn-outline-secondary'?>">Selesai</a>
                    <a href="<?php echo base_url('admin/peminjaman?status=rejected')?>" class="btn btn-sm <?php echo $status == 'rejected' ? 'btn-info' : 'btn-outline-secondary'?>">Rejected</a>
                </div>
            </div>
        </div>
        <div class="card-body">
                    ...
                    <?php if ($peminjamans): ?>
                        <?php foreach ($peminjamans as $i => $row): ?>
                            <tr>
                                ...
                                <td>
                                    <span class="font-monospace"><?php echo $row['kode_transaksi']?></span>
                                </td>
                                <td><?php echo $row['peminjam'] ?? '-'?></td>
                                <td><?php echo date('d-m-Y', strtotime($row['tanggal_pinjam'] ?? $row['created_at']))?></td>
                                <td><?php echo $row['lokasi_pinjam'] ?? '-'?></td>
                               <td>
    <?php
        $status   = $row['status'];
        $mapColor = [
        ...
        <?php echo $statusText?>
                                ...
                                <td>
                                    <div class="btn-group btn-group-sm">
                                        <a href="<?php echo base_url('admin/detailPeminjaman/' . $row['peminjaman_id'])?>"
                                            class="btn btn-outline-primary">
                                            <i class="fa fa-list"></i> Detail
                                        </a>
                                        <?php if ($row['status'] == 'diproses'): ?>
                                            <a href="<?php echo base_url('admin/approvePeminjaman/' . $row['peminjaman_id'])?>"
                                                class="btn btn-success">
                                                <i class="fa fa-check"></i> Approve
                                            </a>
                                            <a href="<?php echo base_url('admin/rejectPeminjaman/' . $row['peminjaman_id'])?>"
                    ...
                    <?php else: ?>
                        <tr>
                            <td colspan="8" class="text-center text-muted py-5">
                                <img src="<?php echo base_url('assets/img/empty-state.svg')?>" alt="" height="80" class="mb-2 d-block mx-auto opacity-50" />
